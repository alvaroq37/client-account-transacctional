...
public class BranchImpl {
    @Inject
    BranchRepository agencyRepository;
    ...
    public Response agencyFindById(JsonObject jsonDataAgency) {
        try {
            ...
            if(id > 0){
                Branch agency = agencyRepository.agencyFindById(id);
                if (agency == null) {
    ...
    public Response agencyFindByName(JsonObject jsonDataAgency) {
        try {
            ...
            if(!name.isEmpty()){
                Branch agency = agencyRepository.agencyFindByName(name);
                if (agency == null) {
                    jsonResponse.put("message", "No existe la agencia solicitada");
                    return Response.ok(jsonResponse).build();
                }
